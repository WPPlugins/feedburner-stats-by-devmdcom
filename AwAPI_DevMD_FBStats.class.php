...
*/
	...
	class AwAPI_DevMD_FBStats{
		/**
		* Feedburner Awareness API Service URL
		* @var string
		* @access private 
		*/
		private $API = 'https://feedburner.google.com/api/awareness/1.0/';
		...
		*/ 
		public function FeedData(){
			...
			if($raw !== false && is_array($raw['ENTRY'][0]) && count($raw['ENTRY'][0]) && is_array($raw['FEED'][0]) && count($raw['FEED'][0]) > 0){
				...
				$this -> error('No basic feed data available!');
				return false;
			}
		}
		...
		*/ 
		public function FeedDataHistory($startDate, $endDate = ''){
			...
			if($raw !== false && is_array($raw['ENTRY']) && count($raw['ENTRY']) > 0){
				...
				$this -> error('No feed history data available!');
				return false;
			}
		}		
		...
		*/ 
		public function ItemData($itemurl = ''){
			...
			if($raw !== false && is_array($raw['ITEM']) && count($raw['ITEM']) > 0){
				...
				$this -> error('No item data available!');
				return false;
			}
		}	
		...
		public function ItemDataHistory($startDate, $endDate = '', $itemurl = ''){
			...
			if(!empty($itemurl)){
				if($raw !== false && is_array($raw['ITEM']) && count($raw['ITEM']) > 0 && is_array($raw['ENTRY']) && count($raw['ENTRY']) > 0){
					...
					$this -> error('No historical data available for this item!');
					return false;
				}
			}
			else{
				if(is_array($raw) && count($raw) > 0){
					...
					$this -> error('No historical item data available!');
					return false;
				}
			}
		}	
		...
		*/ 
		public function ResyndicationData($itemurl = ''){
			...
			if(is_array($raw) && count($raw) > 0){
				...
				$this -> error('No resyndication data available!');
				return false;
			}
		}	
		...
		*/ 		
		private function call($method, $extra = ''){
			...
			if(is_array($extra) && count($extra) > 0){
				...
				$url = $this -> API.$method.'?uri='.$this -> uri.'&'.implode('&', $extra_url);
			}
			else{
				$url = $this -> API.$method.'?uri='.$this -> uri;
			}
			# echo $url;
			...
			if(in_array('curl', get_loaded_extensions())){
				...
				if($header['http_code'] =='401'){
					$this -> errcode = 2;
					$this -> error('401 Unauthorized! This feed does not permit Awareness API access');
				}
			}
			else{
				# No cURL installed use file_get_contents
